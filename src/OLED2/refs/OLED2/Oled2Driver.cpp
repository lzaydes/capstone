/************************************************************************/
/*																		*/
/*	Oled2Driver.c	-- Graphics Driver Library for OLED Display			*/
/*																		*/
/************************************************************************/
...
extern uint8_t	rgbOled2FontUser[];
...
uint8_t *	pbOled2Cur;			//address of byte corresponding to current location
int			bnOled2Cur;			//bit number of bit corresponding to current location
uint8_t		clrOled2Cur;			//drawing color to use
uint8_t *	pbOled2PatCur;		//current fill pattern
int			fOled2CharUpdate;

int			dxcoOled2FontCur;
int			dycoOled2FontCur;

uint8_t *	pbOled2FontCur;
uint8_t *	pbOled2FontUser;
...
*/
void	PMPSendCmd(uint8_t cmd);
void	PMPSendData(uint8_t data);
void	PMPSendDataBuffer(const uint8_t * pbBuf, int cbBuf);
...
Oled2HostTerm()
	...
	///* Make the Data/Command select, Reset, and 
	//*/
	PORTSetPinsDigitalIn(prtDataCmd, bitDataCmd);		
	PORTSetBits(prtDataCmd, bitDataCmd);				
	PORTSetPinsDigitalIn(prtReset, bitReset);		
	PORTSetBits(prtReset, bitReset);						
	PORTSetPinsDigitalIn(prtSelect, bitSelect);	
	PORTSetBits(prtSelect, bitSelect);			
	PORTSetPinsDigitalIn(prtRead, bitRead);			
	PORTSetBits(prtRead, bitRead);						
	PORTSetPinsDigitalIn(prtWrite, bitWrite);						
	PORTSetBits(prtWrite, bitWrite);					

	///* Make power control pins be inputs. The pullup resistors on the
	//** board will ensure that the power supplies stay off.
	//*/
	PORTSetPinsDigitalIn(prtVddCtrl, bitVddCtrl);		//Vdd power control (0=off)
	PORTSetPinsDigitalIn(prtVccCtrl, bitVccCtrl);		//Vcc power control (0=off)
	PORTClearBits(prtVddCtrl, bitVddCtrl);
	PORTClearBits(prtVccCtrl, bitVccCtrl);
...
Oled2DvrInit()
	...
	*/
	...
	dycoOled2FontCur = 8;
	pbOled2FontCur = rgbOledFont0;		// rgbOledFont0 defined in CharFont0.c
	...
	*/
	...
	/* Set the default Gray Scale (GS15) and Fill Pattern
	*/
	clrOled2Cur = 0x0F;
...
Oled2DevTerm()
	...
	*/
	PORTClearBits(prtVccCtrl, bitVccCtrl);
	delay(200); // data sheet says wait at least 100 ms
...
**
**	Parameters:
**		uint8_t* pbBuf - pointer to the display buffer
